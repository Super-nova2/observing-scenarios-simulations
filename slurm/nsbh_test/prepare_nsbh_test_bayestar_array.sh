#!/bin/bash
#SBATCH --job-name=gw-nsbh-test-bayestar-prep
#SBATCH --nodes=1
...
SKYMAP_DIR="${SKYMAP_DIR:-$BASE_DIR/data/skymap/nsbh_skymap_test}"
...
run_dir="runs/$RUNS/nsbh_test_seed${SEED}"
...
echo "NSBH test localization status for $RUNS (seed=$SEED)"
...
if [ "$n_missing" -eq 0 ]; then
  echo "All NSBH test events already have current sky maps; no array submitted."
  exit 0
fi
...
ARRAY_JOB_ID=$(sbatch --parsable \
  ...
  slurm/nsbh_test/run_nsbh_test_bayestar_array.sh)
