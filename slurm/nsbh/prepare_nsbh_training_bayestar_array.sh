...
#   sbatch --export=ALL,RUNS="O5a O5b",SEED=42,EVENTS_PER_TASK=200 slurm/nsbh/prepare_nsbh_training_bayestar_array.sh
RUNS="${RUNS:-O5a}"
SEED="${SEED:-42}"
...
for run in $RUNS; do
  ...
  ARRAY_JOB_ID=$(sbatch --parsable \
    ...
    slurm/nsbh/run_nsbh_training_bayestar_array.sh)
